...
const ENDPOINTS = {
    USERS: `${API_BASE}/api/users/findAll`,
...
async function fetchProfileUrl(profileId) {
    ...
    try {
        const profile = await apiFetch(`${ENDPOINTS.BINARY_CONTENT}?binaryContentId=${profileId}`);
...
function renderPanelChannels(channels) {
    ...
    channels.forEach((ch, i) => {
        ...
        item.innerHTML =
            `<span class="ch-icon">${CHANNEL_ICONS[i % CHANNEL_ICONS.length]}</span>` +
...
async function loadUsers() {
    ...
    try {
        ...
        users.forEach((u, i) => {
            ...
            // 호버 이벤트 → 채널 패널
            card.addEventListener('mouseenter', () => showPanel(card, userId, username));
