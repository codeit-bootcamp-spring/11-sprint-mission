...
class NotificationRequiredEventListenerTest {
  ...
  void on_MessageCreated_NotifiesSubscribers() {
  ...
  @Test
  @DisplayName("메시지 생성 시 구독자가 없으면 알림이 생성되지 않는다")
  void on_MessageCreated_NoSubscribers_NoNotifications() {
    UUID channelId = UUID.randomUUID();
    UUID authorId = UUID.randomUUID();
  ...
  @Test
  @DisplayName("알림 제목은 '작성자명 (#채널명)' 형식이다")
  void on_MessageCreated_TitleFormat() {
    UUID channelId = UUID.randomUUID();
    UUID authorId = UUID.randomUUID();
  ...
  @Test
  @DisplayName("권한 변경 시 대상 유저에게 알림이 생성된다")
  void on_RoleUpdated_CreatesNotification() {
    UUID userId = UUID.randomUUID();
    listener.on(new RoleUpdatedEvent(userId, Role.USER, Role.ADMIN));
    verify(notificationService).create(userId, "권한이 변경되었습니다.", "USER -> ADMIN");
  }

  @Test
  @DisplayName("권한 변경 알림 내용은 '이전권한 -> 새권한' 형식이다")
  void on_RoleUpdated_ContentFormat() {
    UUID userId = UUID.randomUUID();
    listener.on(new RoleUpdatedEvent(userId, Role.ADMIN, Role.USER));
