...
class MessageControllerTest {
  ...
  void createMessage_Success() throws Exception {
    ...
    UserDto author = new UserDto(authorId, "testuser", "test@example.com", null, true, Role.USER);
  ...
  void updateMessage_Success() throws Exception {
    ...
    MessageDto updatedMessage = new MessageDto(
        messageId, now.minusSeconds(60), now,
        "수정된 메시지 내용입니다.",
        channelId, author, new ArrayList<>()
    );
  ...
  void findAllByChannelId_Success() throws Exception {
    ...
    List<MessageDto> messages = List.of(
        new MessageDto(UUID.randomUUID(), cursor.minusSeconds(10), cursor.minusSeconds(10),
            "첫 번째 메시지", channelId, author, new ArrayList<>()),
        new MessageDto(UUID.randomUUID(), cursor.minusSeconds(20), cursor.minusSeconds(20),
            "두 번째 메시지", channelId, author, new ArrayList<>())
    );

    PageResponse<MessageDto> pageResponse = new PageResponse<>(
        messages, cursor.minusSeconds(30), pageable.getPageSize(), true, (long) messages.size()
    );
